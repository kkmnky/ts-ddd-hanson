...
export class ReviewId extends ValueObject<ReviewIdValue, "ReviewId"> {
  ...
  protected validate(value: ReviewIdValue): void {
    if (value.length > ReviewId.MAX_LENGTH || value.length < ReviewId.MIN_LENGTH) {
      throw new Error(`ReviewIdは${ReviewId.MIN_LENGTH}文字以上、${ReviewId.MAX_LENGTH}文字以下でなければなりません。`);
    }
  }
}
